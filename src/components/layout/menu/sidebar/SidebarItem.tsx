  ...
  return (
    ...
    >
      <Link
        ...
        className={classNames("flex items-center p-2 label", {
          "opacity-60": pathname !== routePath,
          "transition-opacity duration-150 ease-in-out": pathname !== routePath,
        })}
      >
        <React.Fragment>{icon}</React.Fragment>
        <span className="ml-2 text-sm font-medium text-black">{text}</span>
      </Link>
      <div
        ref={marker}
        className={classNames(
          "marker absolute start-full bottom-1/2 -translate-x-1 translate-y-1/2 h-6 w-0.5 bg-yellow-400",
          {
